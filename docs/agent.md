...
| `render.set` | `[normal_strength], [occlusion_strength], [bloom_intensity]` | How hard normal maps push, how much baked occlusion is applied, and how strong the bloom is, for the whole scene. Zero on any is the frame this renderer draws without that -- bloom_intensity of zero is the frame with no glow at all -- which is what makes the difference measurable rather than a matter of opinion. |
