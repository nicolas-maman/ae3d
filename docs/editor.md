...
A script links the same engine library the editor links, `build/libae3d_native`,
so a call into the engine reaches the one copy the editor is running rather than
a second copy with an empty GL loader in it. Two of the three platforms would
let a script leave those calls for the host to resolve; Windows would not, and a
script that reached into the engine could not be linked there at all.
