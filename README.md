...
| `black_hole.ae` | Kerr geodesics integrated per pixel in one screen quad: a spinning hole, its asymmetric shadow, a Novikov-Thorne disc lensed over the top of it, a lensed starfield, and redshift from gravity, Doppler and frame dragging as one factor. Orbits by default; SPACE for free flight, 1-4 for rays per pixel ([picture](docs/black-hole.png)) |
