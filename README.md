...
<sub>`AE3D_CROWD=20000 ./build/zombie_city`: the near tier draws the full mesh, the far tier a 168-triangle stand-in; the draw count does not change with the crowd. ~38 fps on an RTX 4070 Ti at 1280×720 with the GPU shared.</sub>
...
tools/       ae3d_agent.py (client), ae3d_bench.ae (frame budget), blender/ (the pipeline)
